...
const AP_Param::GroupInfo AP_Arming::var_info[] = {
    ...
    // index 8 was CHECK in AP_Int32, became SKIPCHK
...
__INITFUNC__ void AP_Arming::init(void)
{
    ...
    if (!checks_to_skip.configured()) {
        // new parameter is not configured
        int32_t skipchk_new = 0; // default to none skipped

        int32_t checks_old;
        if (AP_Param::get_param_by_index(this, 8, AP_PARAM_INT32, &checks_old)) {
            // the old parameter was customized
            if (!(checks_old & 1)) { // ALL flag not set?
        ...
        // set and save to finish migration
        checks_to_skip.set_and_save(skipchk_new);
    }
}
...
{
    return !(checks_to_skip & uint32_t(check));
}
...
bool AP_Arming::arm(AP_Arming::Method method, const bool do_arming_checks)
{
    ...
    if (armed && do_arming_checks && all_checks_skipped()) {
