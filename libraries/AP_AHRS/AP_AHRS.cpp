...
bool AP_AHRS::set_origin_to_recorded()
{
    if (state.origin_ok) {  // we already have an origin
        return true;
    }

    Location loc = { int32_t(_origin_lat.get() * 1e7), int32_t(_origin_lng.get() * 1e7), int32_t(_origin_alt.get() * 100), Location::AltFrame::ABSOLUTE };
