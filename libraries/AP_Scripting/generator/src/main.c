...
int emit_references(const struct argument *arg, const char * tab) {
  ...
  fprintf(source, "%sif (%d > LUA_MINSTACK) {\n", tab, return_count+1);
  fprintf(source, "%s  luaL_checkstack(L, %d, nullptr);\n", tab, return_count+1);
  fprintf(source, "%s}\n\n", tab);
  while (arg != NULL) {
...
void emit_type_index_ex(struct userdata * data, char * meta_name) {
...
void emit_loaders(void) {
  ...
  emit_type_index_ex(parsed_singletons, "singleton");
  ...
  fprintf(source, "                return not_supported_error(L, 1, singleton_fun[i].name);\n");
  fprintf(source, "            }\n");
...
void emit_index_helpers(void) {
  fprintf(source, "static bool load_function(lua_State *L, const luaL_Reg *list, const uint8_t length) {\n");
  fprintf(source, "    const char * name = luaL_checkstring(L, 2);\n");
  ...
  fprintf(source, "static bool load_enum(lua_State *L, const userdata_enum *list, const uint8_t length) {\n");
  fprintf(source, "    const char * name = luaL_checkstring(L, 2);\n");
  fprintf(source, "    for (uint8_t i = 0; i < length; i++) {\n");
  fprintf(source, "        if (strcmp(name,list[i].name) == 0) {\n");
