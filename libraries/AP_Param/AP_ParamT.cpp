...
void AP_ParamTB<T, PT>::set_enable(const T &v) {
...
template<typename T, ap_var_type PT>
void AP_ParamTB<T, PT>::set_default(const T &v) {
...
template<typename T, ap_var_type PT>
void AP_ParamTB<T, PT>::set_and_default(const T &v) {
#if AP_PARAM_DEFAULTS_ENABLED
    add_default(this, (float)v);
#endif
    set(v);
...
template<typename T, ap_var_type PT>
void AP_ParamTB<T, PT>::set_and_notify(const T &v) {
...
template<typename T, ap_var_type PT>
void AP_ParamTB<T, PT>::set_and_save(const T &v) {
...
// updated correctly.
template<typename T, ap_var_type PT>
void AP_ParamTB<T, PT>::set_and_save_ifchanged(const T &v) {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
    if (v == _value) {
#pragma GCC diagnostic pop
...
template<typename T, ap_var_type PT>
float AP_ParamTB<T, PT>::cast_to_float(void) const {
    return (float)_value;
}

template class AP_ParamTB<float, AP_PARAM_FLOAT>;
template class AP_ParamTB<int8_t, AP_PARAM_INT8>;
template class AP_ParamTB<int16_t, AP_PARAM_INT16>;
template class AP_ParamTB<int32_t, AP_PARAM_INT32>;
